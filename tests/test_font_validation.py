...
from banger.fonts import get_font_height, get_character_data, get_available_fonts, \
    get_font_characters
...
class TestFontValidation(unittest.TestCase):
                ...
                for char, char_data in characters.items():
                    with self.subTest(font=font_name, char=repr(char)):
                        ...
                        if (font_name, char) in known_issues:
                            ...
                            self.assertEqual(actual_height, expected_for_known_issue,
    ...
    def test_quadrant_font_has_complete_alphanumeric_set(self):
        ...
        self.assertEqual(len(missing_uppercase), 0,
        ...
        self.assertEqual(len(missing_lowercase), 0,
        ...
        for char, font in test_cases:
            with self.subTest(char=char, font=font):
                ...
                self.assertIsNotNone(char_data,
                                     f"get_character_data('{char}', '{font}') returned None")
                self.assertIn('lines', char_data,
                ...
                self.assertGreater(char_data['width'], 0,
    ...
    def test_quadrant_font_height_consistency(self):
        """Test that quadrant font consistently uses 4-line height."""
        expected_height = 4
        ...
        for char in test_chars:
            with self.subTest(char=char):
                ...
                self.assertIsNotNone(char_data, f"Character '{char}' not found in quadrant font")
                self.assertEqual(len(char_data['lines']), expected_height,
    ...
    def test_quadrant_font_perfect_integrity(self):
        """Test that every single character in quadrant font has exactly 4 lines."""
        expected_height = get_font_height('quadrant')
        characters = get_font_characters('quadrant')

        self.assertEqual(expected_height, 4, "Quadrant font height should be 4")

        # Test every single character
        for char, char_data in characters.items():
            with self.subTest(char=repr(char)):
                self.assertIn('lines', char_data, f"Character '{char}' missing 'lines' key")
                lines = char_data['lines']
                self.assertEqual(len(lines), expected_height,
    ...
    def test_quadrant_font_can_render_full_alphabet_and_numbers(self):
        ...
        for text in test_cases:
            with self.subTest(text=text[:20] + "..." if len(text) > 20 else text):
                ...
                self.assertGreater(len(result), 0, "Rendered output should not be empty")

                # Should have exactly 4 lines of content plus trailing newlines
                lines = result.split('\n')
                content_lines = [line for line in lines if line.strip()]
                self.assertEqual(len(content_lines), 4,
                                 f"Quadrant font should produce exactly 4 content lines, got {len(content_lines)}")
    ...
    def test_fallback_to_default_font_for_invalid_font(self):
        ...
        self.assertEqual(char_data['lines'], default_char_data['lines'],
    ...
    def test_unknown_characters_return_none(self):
        ...
        unknown_chars = ['€', '中', '🎉', '∑']  # Various Unicode characters not in fonts
